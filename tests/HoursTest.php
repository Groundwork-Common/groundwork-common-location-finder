...
final class HoursTest extends TestCase {
	...
	private function field( array $settings = array() ): array {
		return array(
			...
			'type'     => 'hours',
			'label'    => 'Hours',
			'settings' => lfndr_settings_hours( $settings ),
		);
	}
	...
	private function lines( array $slots, array $settings = array() ): array {
		...
		$out   = array();
		foreach ( lfndr_hour_slot_lines( lfndr_sanitize_hours( $slots, $field ), $field ) as $line ) {
	...
	public function test_times_snap_to_the_step(): void {
		$this->assertSame( '09:00', lfndr_sanitize_hour_time( '09:03', 15 ) );
		$this->assertSame( '09:15', lfndr_sanitize_hour_time( '09:08', 15 ) );
		$this->assertSame( '09:30', lfndr_sanitize_hour_time( '09:30', 15 ) );
		// Nearest, not floor: 09:20 is ten minutes from 09:30 and twenty from 09:00.
		$this->assertSame( '09:30', lfndr_sanitize_hour_time( '09:20', 30 ) );
		$this->assertSame( '09:00', lfndr_sanitize_hour_time( '09:10', 30 ) );
	}

	public function test_times_are_zero_padded_and_range_checked(): void {
		$this->assertSame( '09:00', lfndr_sanitize_hour_time( '9:00' ) );
		$this->assertSame( '', lfndr_sanitize_hour_time( '25:00' ) );
		$this->assertSame( '', lfndr_sanitize_hour_time( '09:75' ) );
		$this->assertSame( '', lfndr_sanitize_hour_time( 'noon' ) );
		$this->assertSame( '', lfndr_sanitize_hour_time( '' ) );
	}
	...
	public function test_snapping_never_rolls_past_midnight(): void {
		...
		$this->assertSame( '23:59', lfndr_sanitize_hour_time( '23:58', 15 ) );
	}

	/* ── Slot sanitization ──────────────────────────────────────────────── */

	public function test_slots_without_a_start_are_dropped(): void {
		$slots = lfndr_sanitize_hours(
			array(
				$this->slot( 2, '', '11:00' ),
	...
	public function test_an_end_before_the_start_becomes_open_ended(): void {
		...
		$slots = lfndr_sanitize_hours( array( $this->slot( 2, '21:00', '02:00' ) ), $this->field() );
	...
	public function test_weekly_cannot_be_removed_from_a_field(): void {
		...
		$settings = lfndr_settings_hours( array( 'frequencies' => array( '2nd', '4th' ) ) );
	...
	public function test_an_empty_schedule_produces_no_lines(): void {
		$this->assertSame( array(), lfndr_hour_slot_lines( array(), $this->field() ) );
	}
	...
	public function test_the_payload_ships_raw_slots_alongside_the_lines(): void {
		...
		$field   = $this->field();
		$slots   = lfndr_sanitize_hours( array( $this->slot( 2, '09:00', '11:00' ) ), $field );
		$payload = lfndr_payload_hours( $slots, $field );
