 ...
 * @return string
 */
function lfndr_help_paragraphs( string ...$paragraphs ): string {
	$out = '';
 ...
 */
function lfndr_help_tabs_for( string $tab ): array {
	$overview = array(
		'title'   => __( 'Overview', 'groundwork-common-location-finder' ),
		'content' => lfndr_help_paragraphs(
 ...
 */
function lfndr_add_help_tabs(): void {
	$screen = get_current_screen();
	if ( ! $screen ) {
		return;
	}

	foreach ( lfndr_help_tabs_for( lfndr_current_tab() ) as $i => $tab ) {
		$screen->add_help_tab(
			array(
				'id'      => 'lfndr-help-' . $i,
				'title'   => $tab['title'],
	...
	$screen->set_help_sidebar(
		...
		. '<p><a href="' . esc_url( LFNDR_GWC_URL ) . '" target="_blank" rel="noopener noreferrer">'
